...
// prints as `oracle_pct` (gpqa 98.0 = 194/198, wall 4) — a ceiling, published
// under that name because reading it beside `best_single` invites the one
// conclusion the data cannot support: that routing beat the best arm by 5.1pp.
//
...
// and index.json publishes what it measures as `held_out_pct` beside the ceiling.
//
...
// index.json prints as `oracle_pct`.
...
// what a replay table IS — but it is the number index.json publishes as
// `oracle_pct`, and it belongs next to a test that says why.
